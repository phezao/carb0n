...
class ExamplesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create example" do
    assert_difference('Example.count') do
      post examples_url, params: { example: { address: @example.address, name: @example.name, star: @example.star } }
    end
  ...
  test "should update example" do
    patch example_url(@example), params: { example: { address: @example.address, name: @example.name, star: @example.star } }
